...
export async function POST(req: NextRequest) {
  ...
  const { date, nNameId, action, version = 1 } = await req.json();

  const entry = await prisma.pc_daily_n_entries.findFirst({
  ...
  if (action === "lock") {
    ...
    const updated = await prisma.pc_daily_n_entries.update({
      where: { id: entry.id },
